...
const ConversationSettingsPage = () => {
    ...
    const [freeMessageStatus, setFreeMessageStatus] =
        useState<FreeMessageStatus>({
            userEnabled: false,
            bothEnabled: false,
        });
            ...
            try {
                ...
                if (!data.error && data.receiver) {
                    ...
                    if (data.receiver.id && user?.id !== data.receiver.id) {
                        try {
                            const blockResult = await checkBlockStatus(
                                data.receiver.id,
                            );
                    ...
                    setError(
                        data.message || "Failed to fetch conversation data",
                    );
                }
            } catch (error) {
    ...
    const handleToggleFreeMessage = async () => {
        ...
        try {
            ...
            if (!data.error) {
                ...
                toast.error(
                    data.message || "Failed to update free message setting",
                );
            }
        } catch (error) {
