...
const fetchRecentPosts = async (timeRange) => {
    ...
    switch (timeRange) {
        case '24hrs':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 86400000);
            break;
        case '48hrs':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 172800000);
            break;
        case '3days':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 259200000);
            break;
        case '7days':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 604800000);
            break;
        case '1month':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 2592000000);
            break;
        case '3months':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 7776000000);
            break;
        case '6months':
            filteredPosts = basePosts.filter(post => now - post.timestamp <= 15552000000);
            break;
        case 'alltime':
    ...
    const MetricCard = ({title, value, trend, icon: Icon}) => (
        <div className="bg-white rounded-lg border border-black/10 p-4">
            ...
            <div className="flex items-baseline">
                ...
                <span
                    className={`ml-2 text-xs flex items-center ${trend > 0 ? 'text-green-500' : trend < 0 ? 'text-red-500' : 'text-gray-500'}`}>
          {trend > 0 ? '+' : ''}{trend}%
                    {trend > 0 ? <ArrowUpRight size={12} className="ml-1"/> :
                        trend < 0 ? <ArrowUpRight size={12} className="ml-1 transform rotate-90"/> : null}
        </span>
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-lg border border-black/10 p-4">
                    <h2 className="font-bold text-gray-800 mb-4">Audience Demographics</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        {loading ? renderChartSkeleton() : (
                            <PieChart>
                                <Pie
                ...
                <div className="bg-white rounded-lg border border-black/10 p-4 lg:col-span-3">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center text-sm text-gray-500">
              <span>Last {timeRange === '24hrs' ? '24 Hours' :
                  timeRange === '48hrs' ? '48 Hours' :
                      timeRange === '3days' ? '3 Days' :
                          timeRange === '7days' ? '7 Days' :
                              timeRange === '1month' ? 'Month' :
                                  timeRange === '3months' ? '3 Months' :
                                      timeRange === '6months' ? '6 Months' : 'All Time'}</span>
                        </div>
                    </div>
                    <ResponsiveContainer width="100%" height={300}>
                        {loading ? renderChartSkeleton() : (
                ...
                <div className="bg-white rounded-lg border border-black/10 p-4 lg:col-span-3">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="font-bold text-gray-800">Recent Posts Performance</h2>
                        ...
                        {loading ? (
                            ...
                            <table className="min-w-full">
                                ...
                                <tr className="border-b border-gray-200">
                                    <th className="py-3 px-4 text-left text-sm font-medium text-gray-500">Post</th>
                                    ...
                                    <th className="py-3 px-4 text-left text-sm font-medium text-gray-500">Engagement
                                        Rate
                                    </th>
                                </tr>
                                </thead>
                                ...
                                {recentPostsData.map((post) => (
                                    <tr key={post.id} className="border-b border-gray-200 hover:bg-gray-50">
                                        ...
                                        <td className="py-4 px-4 text-sm">{post.likes.toLocaleString()}</td>
