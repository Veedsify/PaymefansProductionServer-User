...
class SocketManager {
    ...
    private validateConfig(): void {
        if (!this.config.url) {
            throw new Error(
                "Socket URL is not defined in environment variables",
            );
        }
    }

    // Connects the socket with the given username
    public connect({ username, userid }: ConnectParams): Socket | null {
        if (!username || !userid) {
...
export const connectSocket = (params: ConnectParams): Socket | null =>
