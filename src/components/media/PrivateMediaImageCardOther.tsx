  ...
  ({ sort, userdata }: { sort: string; userdata: ProfileUserProps }) => {
    ...
    const fetchMedia = async ({ pageParam = 1 }) => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_TS_EXPRESS_URL}/post/other/private-media/${userdata.id}?page=${pageParam}&limit=${process.env.NEXT_PUBLIC_POST_MEDIA_PER_PAGE}`,
        {
          method: "GET",
          headers: {
...
export const PrivateMediaPanelMediaCard = ({
  media,
  PreviewImageHandler,
  indexId,
...
PrivateMediaImageCardOther.displayName = "PrivateMediaImageCardOther";
export default PrivateMediaImageCardOther;
