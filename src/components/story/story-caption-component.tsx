...
const SlideComponent = ({ story, close }: { story: StoryType; index: number; close: () => void }) => {
     const { addcCaptionToStory, updateCaptionStyle, story: mystory, clearStory } = useStoryStore();
     ...
     return (
          <div className="flex relative flex-col items-center justify-center h-full w-full">
               ...
               <textarea
                    ...
                    className={`p-2 w-full rounded-lg bg-transparent backdrop:blur-sm text-white border-none outline-none text-center text-3xl h-auto font-semibold leading-relaxed absolute resize-none ${story?.captionStyle?.fontFamily}`}></textarea>
          </div>
     )
}
