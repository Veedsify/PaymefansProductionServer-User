...
const PostEditor = ({ posts }: PostEditorProps) => {
    ...
    useEffect(() => {
        if (posts) {
            ...
            if (posts?.post_audience) {
                const audience = capitalizeFirstLetter(posts?.post_audience) as "Public" | "Subscribers" | "Private";
                if (visibility !== audience) { // Only set visibility if it has changed
                    setVisibility(audience);
                    setPostAudience({
                        id: postAudienceData.findIndex((audience) => audience === audience),
                        name: audience,
                        icon: postAudienceData.find((x) => x.name === audience)?.icon || <HiOutlineEye size={20} className="inline" />
                    })
                }
            }
            if (posts?.UserMedia) {
    ...
    }, [posts, setPostText, setVisibility, visibility, capitalizeFirstLetter, postAudienceData]);
    ...
    const handlePostSubmit = async () => {
        ...
        try {
            ...
            const res = await SavePost(formData, posts?.post_id === undefined ? "create" : '', posts?.post_id || "");
