...
const StreamDeckCamera = ({ thisUser, streamData }: { thisUser: AuthUserProps | null, streamData: streamDataProps }) => {
     const {
          useCameraState,
          useMicrophoneState,
          useParticipantCount,
          ...
          const endStream = async () => {
               if (isLive && streamData.user_id === thisUser?.user_id) {
                    swal({
                         ...
                         if (confirmed) {
                              call?.stopLive();
                              axiosInstance.post(`/stream/${streamData.stream_id}/go-live`, {
                                   action: "stop-live"
                              }, {
                                   headers: {
               ...
               <div className="flex-1 w-full h-full border dark:border-slate-800 grid lg:grid-cols-3  dark:text-white relative">
                    ...
                    <div className="border h-full w-full dark:border-slate-800 relative flex flex-col">
                         <div className={`h-full gap-2 w-full`}
                              style={{
                                   flex: '1',
                                   display: 'grid',
                                   gridTemplateColumns: participantCount === 1 ? '1fr' : participantCount === 2 ? '1fr' : participantCount === 3 ? '1fr 1fr' : '1fr 1fr',
                                   gridTemplateRows: participantCount === 1 ? '1fr' : participantCount === 2 ? '1fr 1fr' : participantCount === 3 ? '1fr 1fr' : '1fr 1fr'
                              }}
                         >
                              {me && <ParticipantView participant={me} ParticipantViewUI={null} className='overflow-hidden h-full' />}
                    ...
                    <div className="border h-full dark:border-slate-800 lg:relative lg:block absolute top-0 left-0 w-full">
                         <div className="absolute bottom-0 left-0 w-full bg-video-stream-gradient-white">
                              {[
                                   { imgSrc: "/images/user.png", userName: "@jenna", userComment: "Lorem ipsum dolor, sit amet consectetur adipisicing elit." },
                                   { imgSrc: "/images/user.png", userName: "@jenna", userComment: "Lorem ipsum dolor, sit amet consectetur adipisicing elit." }
                              ].map(({ imgSrc, userName, userComment }, index) => (
                                   <div key={index} className="p-2 md:p-3 flex items-center gap-4">
                                        <Image src={imgSrc} alt="User" width={60} height={60} className="h-12 w-12 rounded-full object-cover border-primary-dark-pink border" />
                                        <div>
                                             <span className="text-xs font-bold block dark:text-white">
                                                  <Link href="/user/username">{userName}</Link>
                                             </span>
                                             <span className="text-xs font-medium dark:text-white">{userComment}</span>
                                        </div>
                                   </div>
                              ))}
