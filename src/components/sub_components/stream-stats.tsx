...
const StreamStats = ({
    ...
    const { views, likes, commentsCount } = LiveStreamSockets({ streamId: streamData.stream_id });
    const { useIsCallLive } = useCallStateHooks()
    const callLive = useIsCallLive()
    return (
        <div
            ...
            <div
                ...
                >
                    {isStatsOpen ? <LucideChevronLeft size={40} /> :
                        <LucideChevronRight size={40} />}
                </button>
            </div>
            <div className="mb-14">
                <Image className="block h-8 w-36" width={150} height={30} priority src="/site/logo2.png" alt="Logo" />
            </div>
            <div className="mb-20">
                <span
                    className="font-semibold mb-2 text-sm text-gray-700 inline-block dark:text-gray-300">Stream Title</span>
                <input type="text" readOnly className="text-2xl font-bold border-none w-full p-1"
            ...
            </div>
            {
                user?.user_id === streamData?.user_id && (
                    <>
                        <div className="mb-3">
                            <span className="font-bold mb-2 text-sm inline-block text-gray-700 dark:text-gray-300">Stream Analytics</span>
                            <div className="flex mb-2">
                                {[
                                    ...
                                    },
                                    {
                                        ...
                                        label: "Comments",
                                        count: commentsCount > 1000 ? numeral(commentsCount).format('0.0a') : commentsCount
                                    }
                        ...
                        <div className="mb-3">
                            <span className="font-bold text-sm mb-2 inline-block text-gray-700 dark:text-gray-300">User Analytics</span>
                            <div className="flex mb-2 flex-wrap">
                                {[
                                    {
                                        icon: <Image width={20} height={20} src="/site/coin.svg"
                                            className="w-auto h-5 aspect-square" alt="Coin" />,
                                        label: "Points",
                                        count: "2.3k"
                                    },
                                    { icon: <LucideUsers size={15} />, label: "Followers", count: "130" }
                                ].map(({ icon, label, count }, index) => (
                                    <div key={index}
            ...
            <div className="border flex flex-col gap-2 border-gray-300 dark:border-slate-400 p-3 flex-1 rounded-lg">
                <div className="flex flex-wrap gap-2">
                    <div className="border w-full p-3 rounded">
                        ...
                        <select name="" onChange={handleCameraChange}
                            className='p-3 text-sm w-full truncate overflow-ellipsis border rounded bg-gray-100'>
                            {devices && devices.length > 0 && devices.map((device, index: number) => (
            ...
            <div>
                {callLive ?
                    <button
                        ...
                        {user?.user_id === streamData?.user_id ? "End Stream" : "Leave Stream"}
                    </button>
                    : <button
