...
const MessageSearch = () => {
    ...
    const fetchSearchResults = async (searchTerm: string) => {
        ...
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_TS_EXPRESS_URL}/conversations/search?q=${searchTerm}`, {
                headers: {
    ...
    return (
        <div>
            <div
                className="flex align-baseline justify-between border dark:text-white border-gray-400 rounded-md p-4 mb-7 w-full">
                <input
            ...
            {openSearch && (
                <div
                    className="fixed animate-in top-0 left-0 z-50 w-full h-full flex flex-col items-center justify-center bg-gray-100/90 dark:bg-gray-950/90 rounded-md duration-300 transition-all ease-in-out">
                    ...
                    <div className="mx-auto max-w-screen-lg p-3 w-full flex-1 py-3">
                        ...
                        {loading &&
                            <div className={"flex justify-center p-3"}>
                                <LucideLoader size={30} className="text-gray-500 dark:text-white animate-spin"/>
                            </div>
                        }
