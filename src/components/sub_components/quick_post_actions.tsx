                    ...
                    {user?.username === options.username ? (
                        ownerOptions.map((option, index) => (
                            <li key={index} className={`py-2 hover:bg-gray-50 dark:hover:bg-slate-900 ${index == ownerOptions.length - 1 ? "" : "border-b dark:border-slate-700"}`}>
                                {option?.func ? (<>
                                    <button onClick={option?.func} className="font-medium dark:text-white text-black flex items-center text-sm py-1 w-full px-3">
                                        {option?.icon}
                                        {option?.name}
                                    </button>
                                </>) : (
                                    <>
                                        <Link href={option?.link ? option?.link : "#"} className="font-medium dark:text-white text-black flex items-center text-sm py-1 px-3">
                                            {option?.icon}
                                            {option?.name}
                                        </Link>
                                    </>
