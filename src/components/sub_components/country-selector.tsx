...
const CountrySelector = ({
    ...
    const [selectedCountry, setSelectedCountry] = useState(
        defaultCountry ||
        (acceptedBankCountries[0]?.countryIso ?? "")
    );
    ...
    const currentCountry =
        acceptedBankCountries.find(
            (c) => c.countryIso === selectedCountry
        ) || acceptedBankCountries[0];
    ...
    // Handle selection
    const handleSelect = (countryIso) => {
        if (countryIso !== selectedCountry) {
            setSelectedCountry(countryIso);
            if (onCountryChange) onCountryChange(countryIso);
        }
        setIsOpen(false);
        setSearchTerm("");
    };
    ...
    useEffect(() => {
        if (currentCountry && !isOpen) {
            ...
            const ariaLive = document.getElementById(
                "country-selector-live"
            );
            if (ariaLive) {
                        ...
                        {filteredCountries.length > 0 ? (
                            filteredCountries.map((country) => (
                                <div
                                    ...
                                    onClick={() =>
                                        handleSelect(country.countryIso)
                                    }
                                    tabIndex={0}
                                    onKeyDown={(e) => {
                                        if (
