...
const ConversationCard = ({ conversation }: { conversation: { conversation: Conversation; conversation_id: string; lastMessage: LastMessage; receiver: { user_id: string; username: string; name: string; profile_image: string; }; }; }) => {
    ...
    const isUnread = !conversation.lastMessage.seen && conversation.lastMessage.sender_id !== user?.user_id;
    ...
    return (
        <div
            onClick={handleClick}
            className={`block border-b ${isUnread ? "bg-messages-unread dark:text-black" : "dark:text-white"} dark:bg-gray-900 mb-3 rounded-md cursor-pointer`}
        >
            <div className="flex items-center gap-2 md:gap-5 p-3 dark:text-white">
                ...
                >
                    ...
                    <div className="absolute right-0 scale-110 bg-white p-1 rounded-full bottom-1">
                        <ActiveProfileTag scale={1.2} userid={conversation.receiver.username} />
                    </div>
                </Link>
                <div className="flex-1">
                    <div className="flex flex-1 text-sm gap-4 mb-2 w-full">
                        ...
                        <div className="flex items-center gap-2 ml-auto">
                            ...
                            {!conversation.lastMessage.seen && <span className="text-white w-2 h-2 bg-primary-dark-pink rounded-2xl block"></span>}
                        </div>
                    </div>
                    <div className="text-sm">
                                ...
                                <div>
                                    ...
                                    {conversation.lastMessage.attachment?.length > 0 && (
                                        <div className="flex gap-1">
                                            {conversation.lastMessage.attachment.slice(0, 6).map((file, index) => (
