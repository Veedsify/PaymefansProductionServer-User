...
import {Send, X, Paperclip, Check, Loader2, MessageCircle, User, Headphones, Star, Clock} from "lucide-react";
...
const ContactSupportModal = () => {
    ...
    return (
        <>
            ...
            >
                <div
                    className="absolute inset-0 bg-gradient-to-r from-purple-700 via-pink-700 to-blue-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                <span className="relative flex items-center gap-3 z-10">
          <MessageCircle size={20} className="group-hover:rotate-12 transition-transform duration-300"/>
          Contact Support
        </span>
                <div
                    className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 skew-x-12"></div>
            </button>
...
const Modal = ({show, setShow}: ContactModalProps) => {
    ...
    useEffect(() => {
        if (
            ...
            const timeout = setTimeout(() => {
                ...
                const shouldSwitchAgent = Math.random() > 0.7 && chatMessages.length > 2;
                const agent = shouldSwitchAgent ? AGENTS[Math.floor(Math.random() * AGENTS.length)] : currentAgent;
    ...
    return (
        <div
            className="fixed inset-0 z-50 bg-black/60 transition-all flex items-center justify-center"
            aria-modal="true"
            role="dialog"
            tabIndex={-1}
        >
                ...
                <AnimatePresence mode="wait" initial={false}>
                        ...
                        >
                            <div className="max-w-2xl mx-auto">
                                ...
                                <form className="space-y-6">
                                    ...
                                    <button
                                        type="submit"
                                        className="w-full group relative px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 font-semibold text-lg overflow-hidden"
                                    >
                    <span className="relative flex items-center justify-center gap-3 z-10">
                      ...
                      Send Message
                                        ...
                                        <div
                                            className="absolute inset-0 bg-gradient-to-r from-purple-700 to-pink-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                    </button>
                                </form>
                            </div>
                        </motion.div>
                    ...
                    {activeTab === "chat" && (
                        ...
                        >
                            ...
                            <div
                                className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-900">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            ...
                                            />
                                            <div
                                                className={`absolute -bottom-1 -right-1 w-4 h-4 ${getStatusColor(currentAgent.status)} rounded-full border-2 border-white`}></div>
                                        </div>
                                        <div>
                                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{currentAgent.name}</h3>
                                            <p className="text-sm text-gray-600 dark:text-gray-400">{currentAgent.role}</p>
                                            <div className="flex items-center gap-1 mt-1">
                                                <Star size={12} className="fill-yellow-400 text-yellow-400"/>
                                                <span className="text-xs text-gray-500">{currentAgent.rating}</span>
                          ...
                          Online
                            ...
                            <div
                                className="flex-1 overflow-y-auto px-6 py-6 space-y-6 bg-gradient-to-b from-gray-50/50 to-white dark:from-gray-900/50 dark:to-gray-800">
                                        ...
                                        {msg.sender === "support" && (
                                            <div className="flex flex-col items-center">
                                                <div className="relative">
                                                    ...
                                                    />
                                                    <div
                                                        className={`absolute -bottom-1 -right-1 w-3 h-3 ${getStatusColor(currentAgent.status)} rounded-full border border-white`}></div>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex flex-col max-w-md">
                                            {msg.sender === "support" && msg.agentName && (
                                                <div className="mb-1 px-1">
                                                    <span
                                                        className="text-xs font-semibold text-gray-700 dark:text-gray-300">{msg.agentName}</span>
                                                    <span
                                                        className="text-xs text-gray-500 dark:text-gray-400 ml-2">{msg.agentRole}</span>
                                                </div>
                                            )}
                                            ...
                                            <div
                                                className={`flex items-center gap-2 mt-2 px-1 ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                                                ...
                                                {msg.sender === "user" &&
                                                    (msg.status === "sending" ? (
                                                        ...
                                                        <div className="flex">
                                                            ...
                                                            <Check size={12} className="text-purple-500 -ml-1"/>
                                                        </div>
                                                    ) : null)}
                                            </div>
                                        </div>
                                ...
                                {isSupportTyping && (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col max-w-xs">
                                            <div className="mb-1 px-1">
                                                <span
                                                    className="text-xs font-semibold text-gray-700 dark:text-gray-300">{currentAgent.name}</span>
                                            </div>
                                            <div
                                                className="px-6 py-4 rounded-2xl bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 shadow-sm rounded-bl-md">
                        <span className="flex gap-1 items-center">
                          <span className="inline-block w-2 h-2 bg-purple-400 rounded-full animate-bounce"></span>
                          <span
                              className="inline-block w-2 h-2 bg-purple-400 rounded-full animate-bounce delay-100"></span>
                          <span
                              className="inline-block w-2 h-2 bg-purple-400 rounded-full animate-bounce delay-200"></span>
                        </span>
                                            </div>
