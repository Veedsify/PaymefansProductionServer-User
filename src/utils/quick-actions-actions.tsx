...
const QuickPostActionHooks = ({ options }: QuickPostActionsProps) => {
    ...
    const handleDelete = (e: MouseEvent<HTMLButtonElement>) => {
        ...
        swal({
            ...
            if (willDelete) {
                try {
                    const deletePost = await axiosInstance.delete(`/post/${options.post_id}`, {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                    });
                    if (deletePost.status === 200) {
                        toast.success("Poof! Your post has been deleted!")
            ...
            <div class="bg-white dark:bg-slate-800 p-5 aspect-video rounded-md">
                ...
                <select class="w-72 mt-2 p-3 rounded-md border-collapse bg-transparent border dark:border-slate-700 dark:bg-slate-900 text-black dark:text-white" name="visibility" id="visibility">
                    ${(user && user.is_model) && user.Model?.verification_status ? (`
                        <option value="public" ${options.post_audience == "public" && 'selected'}>Public</option>
                        <option value="subscribers" ${options.post_audience == "subscribers" && 'selected'}>Subscribers</option>
                        <option value="private" ${options.post_audience == "private" && 'selected'}>Private</option>
                        `) : (`
                        <option value="public" ${options.post_audience == "public" && 'selected'}>Public</option>
                        `)}
                </select>
        ...
        visibility.addEventListener("change", async (e) => {
            try {
                ...
                if (setVisibility.status === 200) {
                    toast.success("Visibility set successfully");
