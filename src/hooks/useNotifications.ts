...
export const useNotifications = (page: string = "1") => {
    ...
    const { addAllNotifications, setTotalNotifications, setHasmore } = useNotificationStore();
    ...
    const markAsReadMutation = useMutation({
        ...
        onMutate: async (notificationId: string) => {
            ...
            const previousNotifications = queryClient.getQueryData(["notifications", page]);
        ...
        onError: (err, notificationId, context) => {
            ...
            if (context?.previousNotifications) {
                queryClient.setQueryData(["notifications", page], context.previousNotifications);
            }
    ...
    useEffect(() => {
        if (notificationsQuery.data && !notificationsQuery.data.error && notificationsQuery.data.data) {
            const { notifications, totalNotifications, hasMore } = notificationsQuery.data.data;
            addAllNotifications(notifications);
            setTotalNotifications(totalNotifications);
            setHasmore(hasMore);
        }
    }, [notificationsQuery.data, addAllNotifications, setTotalNotifications, setHasmore]);
