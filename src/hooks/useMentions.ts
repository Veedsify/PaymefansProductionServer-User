...
export const useMentions = () => {
    ...
    const selectMention = useCallback(
        (mentionedUser: MentionUser, textareaRef: React.RefObject<HTMLTextAreaElement | null>) => {
    ...
    const handleKeyDown = useCallback(
        (e: React.KeyboardEvent<HTMLTextAreaElement>, textareaRef: React.RefObject<HTMLTextAreaElement | null>) => {
            if (showMentions && mentionSuggestions.length > 0) {
                switch (e.key) {
                    ...
                    case 'Tab':
                        e.preventDefault();
                        const newText = selectMention(mentionSuggestions[selectedMentionIndex], textareaRef);
                        return newText;
                    case 'Escape':
                        setShowMentions(false);
                        setMentionQuery('');
