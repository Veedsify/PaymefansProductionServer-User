...
const TaggedUsersButton = ({ story }: TaggedUsersButtonProps) => {
    ...
    return (
        <>
            <div className="absolute z-[400] bottom-2 right-2">
                <motion.button
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    onClick={fetchMentions}
                    disabled={isLoading}
                    className="flex cursor-pointer items-center bg-black/30 px-3 py-2 text-white rounded-full transition-colors disabled:opacity-50"
                >
                    {isLoading ? (
