...
export const useChatMessages = ({ conversationId }: UseChatMessagesProps) => {
    ...
    const searchForSpecificMessage = useCallback(
        async (messageId: string) => {
            try {
                const response = await chat.searchMessages(
                    conversationId,
                    messageId,
                );
    ...
    useEffect(() => {
        ...
        const handleNewMessage = (message: ChatMessage) => {
            console.warn("Received new message via socket:", message);
        ...
        socket.on("message-receiver", handleNewMessage);
        socket.on("message-update", handleMessageUpdate);

        return () => {
            socket.off("message-receiver", handleNewMessage);
