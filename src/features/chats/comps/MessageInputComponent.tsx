...
const linkify = (text: string) => {
    ...
    return text.replace(urlRegex, (url) => {
        ...
        const displayUrl =
            escapedUrl.length > 35
                ? `${escapedUrl.substring(0, 35)}...`
                : escapedUrl;
...
const MessageInputComponent = React.memo(
    ({
        ...
        const areUploadsComplete = useMemo(() => {
            ...
            return messageMediaFiles.every(
                (f) => f.media_state === "completed",
            );
        }, [messageMediaFiles]);
        ...
        const canSendMessageMemo = useMemo(() => {
            ...
            const socketReady = isSocketConnected;
            return (
                hasContent &&
                uploadsComplete &&
                noUploadErrors &&
        ...
        const uploadToS3 = async (
            ...
            try {
                ...
                console.warn(`❌ S3 upload failed for ${media_id}:`, {
                    error: error.message,
        ...
        useEffect(() => {
            ...
            const hasProcessingVideo = messageMediaFiles.some(
                (m) =>
                    m.media_type === "video" && m.media_state === "processing",
            );
                ...
                (event: MessageEvent) => {
                    try {
                        if (event.data) {
                            ...
                            setMessageMediaFiles((prev) => {
                                const mediaFile = prev.find(
                                    (m) => m.media_id === data.mediaId,
                                );
                                if (!mediaFile) {
                                    console.log(
        ...
        useEffect(() => {
            ...
            if (missingVars.length > 0) {
                ...
                console.warn(
                    "⚠️ Video uploads may fail due to missing configuration",
                );
            }
        }, []);
        ...
        const handleSendMessage = useCallback(async () => {
            ...
            try {
                ...
                if (
                    ...
                    toast.error(
                        "Upload failed. Please try uploading your files again.",
                    );
                    return;
                }
            ...
            const handleFileChange = async (e: Event) => {
                ...
                const immediateMediaFiles: MessageMediaFile[] = validFiles.map(
                    (file) => {
                        ...
                        const type: "image" | "video" = file.type.startsWith(
                            "video/",
                        )
                            ? "video"
                            : "image";
                ...
                for (const mediaFile of immediateMediaFiles) {
                    if (mediaFile.media_type === "video" && mediaFile.file) {
                        try {
                            const posterUrl = await GenerateVideoPoster(
                                mediaFile.file,
                            );
                            // Update the specific media file with poster
                            setMessageMediaFiles((prev) =>
                                prev.map((m) =>
                                    m.media_id === mediaFile.media_id
                                        ? { ...m, posterUrl }
                                        : m,
                                ),
                            );
                                        ...
                                        <MessageMediaPreview
                                            ...
                                            onRemove={(id) => {
                                                setMessageMediaFiles((prev) => {
                                                    ...
                                                    // Clean up blob URL
                                                    if (
                                                        fileToRemove?.media_url.startsWith(
                                                            "blob:",
                                                        )
                                                    ) {
                                                        URL.revokeObjectURL(
                                                            fileToRemove.media_url,
                                                        );
                                                    }
                                                    return prev.filter(
                                                        (f) => f.id !== id,
